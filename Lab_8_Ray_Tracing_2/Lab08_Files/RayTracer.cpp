...
glm::vec3 trace(Ray ray, int step)
{
    ...
    if (ray.index == 4) //index of plane in SceneObjects list (pushed scene objects in initaliize function)
    {
        //Stripe pattern
        int stripeWidth = 5;
        ...
        float texcoord_s = (ray.hit.x - a1) / (a2 - a1);
        float texcoord_t = (ray.hit.z - ZMIN) / (ZMAX - ZMIN);
        if (texcoord_s > 0 && texcoord_s < 1 &&
            texcoord_t > 0 && texcoord_t < 1)
        {
